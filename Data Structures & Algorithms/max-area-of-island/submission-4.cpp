        ...
        for(int i=0; i<ROWS; i++){
            for(int j=0; j<COLS; j++) {
                ...
                if(grid[i][j] == 1) {
                    ...
                    // if(j-1 >=0 && grid[i][j-1] == '1' && ds.unionsize(idx, getidx(i, j-1))) res--;
                    if(i+1 < ROWS && grid[i+1][j] == 1 && ds.unionsize(idx, getidx(i+1, j))) res--;
                    // if(i-1 >= 0 && grid[i-1][j] == '1' && ds.unionsize(idx, getidx(i-1, j))) res--;
                }
            }
        }
        int maxarea = 0;
        for(int i=0; i<ds.dssize(); i++) {
            int k = i/COLS, l=i%COLS;
            if(grid[k][l] && i == ds.findParent(i)) {
                maxarea = max(maxarea, ds.getcomponentsize(i));
            }
        }
        return maxarea;
    }
